...
namespace Intersect.Client.Core
{
    ...
    public static class Main
    {
        ...
        public static void Start(byte[] rsaKey, Light2D sunLight, UnityEngine.AudioSource musicSource, UnityEngine.Audio.AudioMixer audioMixer)
        {
            Graphics.InitGraphics(sunLight);
            Audio.Init(musicSource, audioMixer);
            Audio.PlayMusic(ClientConfiguration.Instance.MenuMusic, 3, 3, true);

            Networking.Network.InitNetwork(rsaKey);
            Fade.FadeIn();

            Guid id = Guid.NewGuid();
            foreach (object val in Enum.GetValues(typeof(GameObjectType)))
            {
                GameObjectType type = (GameObjectType)val;
        ...
        private static void ProcessMenu()
        {
            ...
            Interface.Interface.GameUi.EscapeMenu.Hide();
        }
        ...
        private static void ProcessGame()
        {
            ...
            {
                Logout(false);
                Interface.Interface.MsgboxErrors.Add(new KeyValuePair<string, string>(null, Strings.Errors.lostconnection));
            ...
            {
                //Update All Entities
                foreach (KeyValuePair<Guid, Client.Entities.Entity> en in Globals.Entities)
                {
                    if (en.Value == null)
                    {
                        continue;
                ...
                //Update Maps
                Models.IDatabaseObject[] maps = MapInstance.Lookup.Values.ToArray();
            ...
            List<Guid> removeHolds = new List<Guid>();
            foreach (KeyValuePair<Guid, Guid> hold in Globals.EventHolds)
            {
            ...
            foreach (Guid hold in removeHolds)
            {
        ...
        public static void Logout(bool characterSelect)
        {
            ...
            foreach (KeyValuePair<Guid, Models.IDatabaseObject> map in MapInstance.Lookup)
            {
                MapInstance mp = (MapInstance)map.Value;
                mp.Dispose(false, true);
            }

            foreach (KeyValuePair<Guid, Entities.Entity> en in Globals.Entities.ToArray())
            {
                en.Value.Dispose();
            }
